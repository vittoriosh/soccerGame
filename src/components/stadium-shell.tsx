...
export function StadiumShell({
  ...
  return (
    <div
      className={`${barlow.className} relative flex min-h-dvh flex-1 text-white ${
        fill
    ...
    >
      ...
      <div
        className={`relative z-10 flex w-full flex-1 flex-col ${
          align === "center"
            ? "items-center justify-center px-4 py-[max(1.5rem,env(safe-area-inset-top))] sm:px-6"
            : fill
              ? "min-h-0 px-3 pt-[max(0.75rem,env(safe-area-inset-top))] pb-[max(0.75rem,env(safe-area-inset-bottom))] sm:px-6"
              : scrollable
